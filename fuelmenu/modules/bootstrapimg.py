...
class bootstrapimg(urwid.WidgetWrap):
    ...
    def responses(self):
        ...
        for index, fieldname in enumerate(self.fields):
            if fieldname == BLANK_KEY or 'button' in fieldname.lower():
                ...
                flavor = 'ubuntu' if rb_group[0].state else 'centos'
    ...
    def _ui_set_bootstrap_flavor(self):
        ...
        is_centos = self._bootstrap_flavor is not None and \
            'centos' in self._bootstrap_flavor
        try:
            rb_group = self.edits[rb_index].rb_group
            rb_group[0].set_state(not is_centos)
            rb_group[1].set_state(is_centos)
        except AttributeError:
            # the UI hasn't been initalized yet
            pass

    def _set_bootstrap_flavor(self, flavor):
        is_centos = flavor is not None and 'centos' in flavor.lower()
        self._bootstrap_flavor = 'centos' if is_centos else 'ubuntu'
