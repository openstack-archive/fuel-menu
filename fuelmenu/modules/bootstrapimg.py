...
class bootstrapimg(urwid.WidgetWrap):
    ...
    def check_apt_repos(self, responses):
        ...
        if BOOTSTRAP_MIRROR_DISTRO_KEY in responses:
            ...
            distro_repos = responses[BOOTSTRAP_MIRROR_DISTRO_KEY]
            if not distro_repos[0]['uri']:
                errors.append('Ubuntu mirror is necessary.')
            else:
                main_repos += distro_repos
        if BOOTSTRAP_MIRROR_MOS_KEY in responses:
            mos_repos = responses[BOOTSTRAP_MIRROR_MOS_KEY]
            if not mos_repos[0]['uri']:
                errors.append('MOS mirror is necessary.')
            else:
                main_repos += mos_repos
        ...
        for index, repo in enumerate(repos + main_repos):
            ...
            if not all((repo['type'], repo['uri'], repo['suite'])):
                errors.append("Cannot parse extra repository {0}. "
                              ...
                              .format(name))
                continue
            repo_url = '{0}/dists/{1}/Release'.format(repo['uri'],
                                                      repo['suite'])
            if not self.check_url(repo_url, proxies):
