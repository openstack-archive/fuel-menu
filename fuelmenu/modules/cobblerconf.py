...
interface first.")
    ...
    def update_dhcp(self):
        ...
        if not result:
            ModuleHelper.display_dialog(
                self, self.apply_dialog_message["message"],
                self.apply_dialog_message["title"])
            return False
    ...
    def _update_hiera_dnsmasq(self, settings):
        ...
        with open(consts.HIERA_NET_SETTINGS, "r") as hiera_settings:
            networks = yaml.load(hiera_settings)
        ...
        with open(consts.HIERA_NET_SETTINGS, "w") as hiera_settings:
            yaml.dump(networks, hiera_settings)
    ...
    def _update_dnsmasq(self, settings):
        ...
        log.debug("Start puppet with data {0}".format(puppetclasses))
