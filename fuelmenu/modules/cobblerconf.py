...
interface first.")
    ...
    def update_dhcp(self):
        ...
        nailgun_updated = self.update_nailgun(responses)
        if not nailgun_updated:
            return False
        if os.path.exists(consts.HIERA_NET_SETTINGS):
            result, msg = self.update_hiera_dnsmasq(responses)
        else:
            result, msg = self.update_dnsmasq(responses)
        if not result:
    ...
    def _update_nailgun(self, responses):
        ...
        gateway = {"gateway": responses["dhcp_gateway"]}
        try:
