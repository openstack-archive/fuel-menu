...
(example 192.168.1.1)",
    ...
    def check(self, args):
        ...
        if responses["ipaddr"] in parent_ips:
            affected_ifaces = [k for k, v in self.netsettings.items()
                               if v['addr'] == responses["ipaddr"]]
            errors.append("IP address already assigned to interface ({0})"
                          .format(affected_ifaces[0]))
