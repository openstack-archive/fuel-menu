...
class TestUtils(unittest.TestCase):
    ...
    def test_interface_is_up_false(self, addrs_mock):
        iface = 'eth0'
        addrs_mock.return_value = {'some_key': 'some_value'}
        self.assertFalse(network.is_interface_up(iface))
        addrs_mock.assert_called_once_with(iface)

    @mock.patch('netifaces.ifaddresses')
    def test_interface_is_up_true(self, addrs_mock):
        iface = 'eth0'
        addrs_mock.return_value = {netifaces.AF_INET: '192.168.1.2'}
        self.assertTrue(network.is_interface_up(iface))
        addrs_mock.assert_called_once_with(iface)
