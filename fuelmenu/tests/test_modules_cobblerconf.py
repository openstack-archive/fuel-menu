...
class TestCobblerConfig(base.BaseModuleTests):
    ...
    def setUp(self):
        ...
        self.get_physical_ifaces = mock.patch(
            "fuelmenu.common.network.get_physical_ifaces",
            return_value=["eth0"])
        self.m_get_physical_ifaces = self.get_physical_ifaces.start()

        self.getDHCP = mock.patch(
            "fuelmenu.modules.cobblerconf.CobblerConfig.getDHCP",
            return_value=False)
        self.m_getDHCP = self.getDHCP.start()

        self._get_net = mock.patch(
            "fuelmenu.common.modulehelper.ModuleHelper._get_net",
            return_value=self.net)
        self.m_get_net = self._get_net.start()

        self.get_default_gateway_linux = mock.patch(
            "fuelmenu.modules.cobblerconf.CobblerConfig."
            "get_default_gateway_linux", return_value="192.168.133.1")

        self.m_get_default_gateway_linux = \
            self.get_default_gateway_linux.start()
        self.responses = {
        ...
        self.is_post_d = mock.patch("fuelmenu.common.utils.is_post_deployment",
                                    return_value=False)
        self.m_is_post_d = self.is_post_d.start()
        self.search_external_dhcp = mock.patch(
            "fuelmenu.common.network.search_external_dhcp",
            return_value=[])
        self.m_search_external_dhcp = self.search_external_dhcp.start()
        self.duplicateIPExists = mock.patch(
            "fuelmenu.common.network.duplicateIPExists", return_value=False)
        self.m_duplicateIPExists = self.duplicateIPExists.start()
        self.get_physical_ifaces = mock.patch(
            "fuelmenu.common.modulehelper.network.get_physical_ifaces",
            return_value=["eth0"])
        self.m_get_physical_ifaces = self.get_physical_ifaces.start()
        self.MH_display_failed = mock.patch(
            "fuelmenu.common.modulehelper.ModuleHelper."
            "display_failed_check_dialog")
        self.m_MH_display_failed = self.MH_display_failed.start()

    def tearDown(self):
        self.is_post_d.stop()
        self.search_external_dhcp.stop()
        self.duplicateIPExists.stop()
        self._get_net.stop()
        self.get_physical_ifaces.stop()
        self.get_default_gateway_linux.stop()
        self.getDHCP.stop()
        self.MH_display_failed.stop()
    ...
    def test_check(self):
        ...
        self.m_get_physical_ifaces.assert_called_once_with()
