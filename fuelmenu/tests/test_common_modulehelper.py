...
def custom_mock_open(lines):
    ...
    m_open.return_value.__iter__ = Mock(return_value=lines.__iter__())
    m_open.return_value.readlines = Mock(return_value=lines)
    return m_open
...
class TestGetDefaultGateway(TestModuleHelperBase):
    ...
    def _check_default_gateway(self, expected):
        with patch('__builtin__.open', custom_mock_open(self.data)):
