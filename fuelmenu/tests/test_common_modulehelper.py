...
@patch('fuelmenu.settings.Settings.read', side_effect=lambda x: x)
@patch('fuelmenu.common.modulehelper.ModuleHelper.get_setting',
       return_value='loaded')
class TestModuleHelperLoad(TestModuleHelperBase):
    def setUp(self):
        super(TestModuleHelperLoad, self).setUp()
        self.modobj.defaults = dict()
        self.modobj.parent = Mock()
    ...
    def test_load_types_skipping(self, *_):
        widget_types = {
            WidgetType.BUTTON: 'skipped',
            WidgetType.CHECKBOX: 'loaded',
            WidgetType.LABEL: 'skipped',
            WidgetType.LIST: 'loaded',
            WidgetType.RADIO: 'loaded',
            WidgetType.TEXT_FIELD: 'loaded'
        }
...
@patch('fuelmenu.common.modulehelper.ModuleHelper.set_setting',
       return_value='')
class TestModuleHelperSave(TestModuleHelperBase):
    def setUp(self):
        super(TestModuleHelperSave, self).setUp()
        self.modobj.oldsettings = Mock()
...
@patch('fuelmenu.common.urwidwrapper.SimpleListWalker',
       return_value=Mock())
@patch('fuelmenu.common.urwidwrapper.WalkerStoredListBox',
       return_value=Mock())
@patch('urwid.BoxAdapter', return_value=Mock())
@patch('fuelmenu.common.modulehelper.ModuleHelper._create_widget',
       return_value=Mock(rows=Mock(return_value=0)))
@patch('urwid.Text', return_value=Mock(rows=Mock(return_value=0)))
class TestModuleHelperCreateListWidget(TestModuleHelperBase):
    def setUp(self):
        super(TestModuleHelperCreateListWidget, self).setUp()
        self.toolbar = Mock()
        modulehelper.blank = Mock()
        modulehelper.blank.rows = Mock(return_value=0)
    ...
    def test_create_list_widget(
        ...
        m_create_widget.assert_has_calls([
            call('l1', {'value': 'vl1'}, self.toolbar),
            call('l1', {'value': 'vl2'}, self.toolbar)
        ])
        # Called twice for two elements and third on exit
        m_listbox.assert_has_calls(
            [call(m_walker.return_value)] * 3
        )
        # Called twice for two elements and third on exit
        m_walker.assert_has_calls(
            [call([m_create_widget.return_value, modulehelper.blank])] * 2 +
            [call([m_text.return_value] + [m_box.return_value] * 2)]
        )
        # Called twice for two elements and third on exit
        m_box.assert_has_calls([call(m_listbox.return_value, 0)] * 3)
...
@patch('fuelmenu.common.urwidwrapper.TextLabel', return_value='text_label')
@patch('fuelmenu.common.urwidwrapper.ChoicesGroup',
       return_value=Mock())
@patch('fuelmenu.common.urwidwrapper.Columns', return_value=Mock())
...
@patch('fuelmenu.common.urwidwrapper.Button', return_value=Mock())
@patch('fuelmenu.common.urwidwrapper.Columns', return_value=Mock())
...
@patch('fuelmenu.common.urwidwrapper.CheckBox', return_value=Mock())
...
@patch('fuelmenu.common.urwidwrapper.TextField', return_value=Mock())
...
class TestModuleHelperCheckColumns(TestModuleHelperBase):
    def setUp(self):
        ...
        self.modobj.check = Mock()
        self.modobj.cancel = Mock()
        self.modobj.apply = Mock()
        self.modobj.parent = Mock(globalsave=False)
        modulehelper.blank = Mock()
    ...
    def test_get_check_column_all_buttons(self, m_button, m_columns):
        ...
        m_button.assert_has_calls(
            [
                call("Check", self.modobj.check),
                call("Cancel", self.modobj.cancel),
                call("Apply", self.modobj.apply)
            ],
            any_order=False
        )
        m_columns.assert_called_once_with(
...
class TestModuleHelper(TestModuleHelperBase):
    ...
    def test_create_widget(self):
        ...
        for widget_type, mock_function, arguments in cases:
            with patch('fuelmenu.common.%s' % mock_function,
                       return_value=Mock()) as m_function:
    ...
    @patch('fuelmenu.common.urwidwrapper.TextLabel', return_value='txt')
    @patch('fuelmenu.common.dialog.display_dialog')
    ...
    @patch('fuelmenu.common.modulehelper.ModuleHelper.display_dialog')
    ...
    @patch('urwid.Text', return_value=Mock())
    ...
    @patch('fuelmenu.common.modulehelper.ModuleHelper._create_widget',
           return_value=Mock())
    def test_get_edits(self, m_create_widget):
        toolbar = Mock()
        fields = ['key1', 'key2']
        value = Mock()
        defaults = {'key1': value}
    ...
    def test_get_network(self):
        ...
        self.modobj.getDHCP = Mock()
        self.modobj.gateway = None
        self.modobj.get_default_gateway_linux = Mock(return_value=Mock())
        self._run('getNetwork', self.modobj)

        self.assertEqual(
            self.modobj.gateway,
            self.modobj.get_default_gateway_linux.return_value)
        self.assertEqual(self.modobj.netsettings, expected)


@patch('fuelmenu.common.modulehelper.log.debug')
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_header_content',
       return_value=['header'])
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_edits',
       return_value=['edit'])
@patch('fuelmenu.common.urwidwrapper.TabbedListWalker',
       return_value=Mock())
@patch('urwid.ListBox', return_value=Mock())
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_check_column',
       return_value='buttons')
class TestScreenUI(TestModuleHelperBase):
    def setUp(self):
        super(TestScreenUI, self).setUp()
        self.modobj.name = 'test'
        self.modobj.parent = Mock(footer=Mock())
        self.header_text = Mock()
        self.fields = Mock()
        self.defaults = Mock()
    ...
    def test_screen_ui(
            ...
            m_get_edits, m_get_header_content, m_debug):
        self._check(
            'screenUI',
        ...
        self.assertEqual(self.modobj.edits, m_get_edits.return_value)
        ...
        m_get_edits.assert_called_once_with(
...
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_iface_info',)
class TestGetInterfaceSettings(TestNetworkMethodsBase):
    ...
    def _check_get_iface_settings(self, m_get_iface_info, expected):
        ...
        m_get_iface_info.assert_has_calls([
            call(self.iface, netifaces.AF_INET),
            call(self.iface, netifaces.AF_LINK)])
...
@patch('__builtin__.open',
       return_value=Mock(
        ...
        __enter__=Mock(return_value=Mock(
                readlines=Mock(return_value=['  down  '])))))
class TestGetLinkState(TestNetworkMethodsBase):
    def setUp(self):
        super(TestGetLinkState, self).setUp()
        self.addr = '127.0.0.1'

    def _check_get_link_state(self, m_open, expected):
    ...
    def test_get_link_state(self, m_open):
        self._check_get_link_state(m_open, 'down')

    @patch('fuelmenu.common.modulehelper.log.warning')
    def test_get_link_state_os_error(self, m_warning, m_open):
        m_open.side_effect = IOError
        self._check_get_link_state(m_open, 'up')
        self._assert_warning(m_warning)

    @patch('fuelmenu.common.modulehelper.log.warning')
    def test_get_link_state_os_error_and_empty_addr(self, m_warning, m_open):
        m_open.side_effect = IOError
        self.addr = ''
        self._check_get_link_state(m_open, 'unknown')
        self._assert_warning(m_warning)
...
class TestGetBootProto(TestNetworkMethodsBase):
    ...
    def _check_get_boot_proto(self, m_open, expected):
        self._check('_get_boot_proto', expected, self.iface, self.dhcp_exist)
        m_open.assert_called_once_with(
            "/etc/sysconfig/network-scripts/ifcfg-%s" % self.iface)

    def test_get_boot_proto(self, m_open):
        self._check_get_boot_proto(m_open, 'bootp')

    def test_get_boot_proto_exception(self, m_open):
        m_open.side_effect = Exception
        self._check_get_boot_proto(m_open, 'dhcp')

    def test_get_boot_proto_exception_and_no_dhcp(self, m_open):
        m_open.side_effect = Exception
        self.dhcp_exist = False
        self._check_get_boot_proto(m_open, 'none')


@patch('fuelmenu.common.modulehelper.ModuleHelper._get_iface_settings',
       return_value=dict(addr='127.0.0.1'))
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_link_state',
       return_value=Mock())
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_boot_proto',
       return_value=Mock())
...
@patch('fuelmenu.common.network.get_physical_ifaces',
       return_values=['eth0', 'eth1'])
@patch('fuelmenu.common.modulehelper.ModuleHelper._get_net',
       return_value=Mock())
class TestGetNetwork(TestModuleHelperBase):
    def setUp(self):
        super(TestGetNetwork, self).setUp()
        self.modobj.getDHCP = Mock(return_value=Mock())
        self.modobj.netsettings = dict()
        self.modobj.gateway = Mock()
        self.modobj.get_default_gateway_linux = Mock(
            return_value=Mock())
    ...
    def test_get_network(self, m_get_net, m_get_physical_ifaces):
        ...
        self.modobj.getDHCP.assert_has_calls(
            [call(iface) for iface in m_get_physical_ifaces.return_value]
        )
        m_get_net.assert_has_calls(
            [call(iface, self.modobj.getDHCP.return_value)
...
@patch('fuelmenu.common.modulehelper.execute')
class TestGetDhcp(TestNetworkMethodsBase):
    ...
    def test_get_dhcp(self, m_execute):
        m_execute.return_value = (0, Mock(), Mock())
        self._check_get_dhcp(True, m_execute)

    def test_get_dhcp_failure(self, m_execute):
        m_execute.return_value = (1, Mock(), Mock())
        self._check_get_dhcp(False, m_execute)


@patch('socket.inet_ntoa', return_value=Mock())
@patch('struct.pack', return_value=Mock())
@patch('__builtin__.open',
       return_value=Mock(
        __exit__=Mock(),
        __enter__=Mock(return_value=[
...
class TestGetDefaultGateway(TestModuleHelperBase):
    def test_get_default_gateway_linux(self, m_open, m_pack, m_inet_ntoa):
        self._check('get_default_gateway_linux', m_inet_ntoa.return_value)

    def test_get_default_gateway_linux_not_found(
            self, m_open, m_pack, m_inet_ntoa):
        del m_open.return_value.__enter__.return_value[3]
        self._check('get_default_gateway_linux', None)
